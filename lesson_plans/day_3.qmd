...
assert len(read_file( \
  "day_3.qml"))
assert is_string(read_file( \
  "day_3.qml")[0])
assert is_text(read_file( \
  "day_3.qml"))
```
...
Create a function that returns this text as a list of strings:
...
In this course, we do not care about speed.

Without runtime speed profiles, any discussion
on this is useless.
...
Would it be possible to express this better?

## Answer

Yes, using a Python set.
